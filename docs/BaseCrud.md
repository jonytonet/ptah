...
29. [Configurable JOINs](#configurable-joins)
30. [Lifecycle Hooks](#lifecycle-hooks)
31. [CRUD Notifications](#crud-notifications)
...
36. [Multi-Config per Route](#multi-config-per-route)
    - [Locking rows to a fixed scope (`lockedFilters`)](#locking-rows-to-a-fixed-scope-lockedfilters)
37. [Deploying a screen: the config lives in the database](#deploying-a-screen-the-config-lives-in-the-database)
38. [Making a new screen appear in the menu](#making-a-new-screen-appear-in-the-menu)
39. [Testing a BaseCrud screen](#testing-a-basecrud-screen)
...
> **`initialFilter` discards the operator.** Each triple is read as `[$field, , $value]` and
> stored as `filters[$field] = $value`; the operator actually applied comes from that
> column's own filter definition in the config. So `['price', '>', 100]` does **not**
> produce `price > 100` — it seeds the `price` filter with `100` and the configured
...
### Locking rows to a fixed scope (`lockedFilters`)

A route-specific config changes *how* a screen looks (columns, totals, read-only).
`lockedFilters` changes *which rows* it can ever show — it restricts EVERY query the
screen runs to a fixed set of column values. It is `#[Locked]` (set once at `mount()`),
so a forged or manipulated request **cannot escape it**.

This is how the same model powers both a global admin list and a per-user scoped view.

**Where the lock is enforced.** Not only the visible rows: every read path goes through one
of two guarded queries, so no number can ever disagree with the scope.

| Path | Guarded by |
|---|---|
| Listing rows | `buildBaseQuery()` |
| Totalizadores | `buildBaseQuery()` — the same filtered query as the listing |
| Export (sync and queued) and print | `buildBaseQuery()` |
| Bulk actions, edit, delete, restore | `scopedQuery()` / `recordInScope()` |

Column names pass `SqlIdentifier::isSafe()` before reaching the `where`.

**`lockedFilters` vs `initialFilter` vs `companyFilter`:**

| Parameter | Purpose | Can the user change/clear it? | Scope |
|---|---|---|---|
| `initialFilter` | A **default** filter to start on | **Yes** — it writes into `$filters`, a client-writable property | any column(s) |
| `companyFilter` | Multi-tenant company scope | No (server-set) | `company_id` only |
| `lockedFilters` | A **hard, immutable** scope | **No** — re-applied on every query | any column(s) |

For anything security-sensitive (tenant, owner, "only mine") use `lockedFilters`, never
`initialFilter`: clearing the filter panel, or a hand-crafted Livewire payload, drops an
`initialFilter` and reveals every row.

```blade
{{-- "My attendances": SAME model as the admin appointment list, but locked to
     the logged-in professional. Pair it with a route-specific config that
     defines this screen's columns + totalizador. --}}
@php
    $employeeId = app(\App\Services\Scheduling\EmployeeService::class)->idForUser(auth()->id());
@endphp

@livewire('ptah-base-crud', [
    'model' => 'Scheduling/Appointment',
    'lockedFilters' => ['employee_id' => $employeeId],
])
```

> **Security:** always compute the scoping value server-side (controller, route closure, or
> a `@php` block as above) and pass it in — never read it from the request. `#[Locked]`
> stops the client tampering with it after mount, but it cannot make a request-supplied
> value trustworthy in the first place.
...
@livewire('ptah-base-crud', [
    'model' => 'Scheduling/Appointment',
    'lockedFilters' => ['employee_id' => $employeeId, 'branch_id' => $branchId],
])
```

---

## Deploying a screen: the config lives in the database

A `crud_configs` row is **data, not a file**, so deploying your code does not carry it.
This catches people out precisely because everything else about the screen — the route,
the Blade view, the model — is in git.

The workflow that keeps the two in sync:

```bash
# 1. Development: export every config to versionable JSON
php artisan ptah:config:export-all          # -> database/ptah/crud-configs/*.json

# 2. Commit those JSON files alongside the code that needs them

# 3. Deploy: register them in the target database
php artisan ptah:config:import-all          # reads database/ptah/crud-configs
```

### Shipping ONE new config without touching the others

`ptah:config:import-all` takes an optional `{path?}` and imports **every `*.json` in that
directory**, calling `save()` (an upsert on model + route) per file. It never truncates the
table. So a directory holding only the new config is a surgical, non-destructive deploy —
anything edited through the visual modal in production stays untouched:

```bash
mkdir -p database/ptah/deploy-2026-09
cp database/ptah/crud-configs/appointment@health-meus-atendimentos.json \
   database/ptah/deploy-2026-09/

php artisan ptah:config:import-all database/ptah/deploy-2026-09
```

Running the plain `import-all` over the whole folder would instead overwrite **every**
config with the committed snapshot, discarding any production edit made since the export.
Prefer the single-directory form whenever you are adding rather than restoring.

---

## Making a new screen appear in the menu

The sidebar is database-driven, and `database/seeders/MenuRegistry.php` in the host app is
the canonical source. A route plus a config is not enough — a new screen stays invisible
until the registry is synced into the `menus` table:

```bash
# Adding entries: non-destructive, keeps rows edited through the menu UI
php artisan ptah:menu-sync
```

`ptah:forge Module/Entity` appends an entry to `MenuRegistry.php` automatically (pass
`--no-menu` to skip); a screen wired by hand needs the entry added by hand.

> **`--fresh` clears the `menus` table before syncing** — the command's own help marks it
> destructive. It is the right call when the registry is the only source of truth (a first
> install, or a deliberate rebuild) and the wrong one for adding a screen to a live system,
> where it discards every menu row created or reordered through the UI. Use plain
> `ptah:menu-sync` unless you mean to rebuild the whole menu.

---

## Testing a BaseCrud screen

BaseCrud renders from data: columns, filters and totals all come from the `crud_configs`
row for that model and route. A test that mounts the component against an empty
`crud_configs` table therefore renders almost nothing, and asserting on the markup tells
you very little.

Two approaches, both used in this package's own suite:

**1. Seed the config, then assert on behaviour.** Write the row directly (or import a JSON
export) and drive the component:

```php
CrudConfig::updateOrCreate(
    ['model' => Appointment::class, 'route' => ''],
    ['config' => [
        'crud' => Appointment::class,
        'cols' => [
            ['colsNomeFisico' => 'title', 'colsNomeLogico' => 'Title', 'colsTipo' => 'text'],
        ],
        'permissions' => [],
    ]]
);

Livewire::test('ptah-base-crud', ['model' => Appointment::class])->assertOk();
```

**2. Assert on the returned rows, not the HTML.** For scoping, ordering and filtering the
useful assertion is which records came back, independent of how they are rendered:

```php
$component = Livewire::test('ptah-base-crud', [
    'model' => Appointment::class,
    'lockedFilters' => ['employee_id' => 7],
]);

$ids = collect($component->instance()->rows()->items())->pluck('id')->all();

$this->assertEqualsCanonicalizing([$mine->id], $ids);
```

See `tests/Feature/Crud/CrudLockedFiltersScopeTest.php` for a worked example that also
covers the totalizador and the escape attempts.
