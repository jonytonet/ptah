...
return [
    ...
    */
    ...
    'guide_faq_a5' => 'Yes, by generation: the cache key embeds global and per-user counters, bumped when a Role/RolePermission/UserRole/PageObject/Page changes. Invalidation is instant on any cache driver — including bulk revocations. <code class="ptah-c-kbd border rounded px-1 text-xs font-mono">php artisan cache:clear</code> is never required in the normal flow.',
    'guide_faq_q6' => 'Can I create Pages and Objects automatically via code?',
    'guide_faq_a6' => 'Yes. Use the seeder or create records in <code class="ptah-c-kbd border rounded px-1 text-xs font-mono">Ptah\Models\PtahPage</code> and <code class="ptah-c-kbd border rounded px-1 text-xs font-mono">Ptah\Models\PageObject</code> directly (not <code class="ptah-c-kbd border rounded px-1 text-xs font-mono">Ptah\Models\Page</code> — that class does not exist). Prefer a seeder or <code class="ptah-c-kbd border rounded px-1 text-xs font-mono">php artisan ptah:permission:sync</code> over a data migration: the package\'s migrations are auto-discovered and fire on any host\'s <code class="ptah-c-kbd border rounded px-1 text-xs font-mono">php artisan migrate</code>, even when the feature is unused.',
