...
return [
    ...
    */
    ...
    'guide_faq_a5' => 'Sim, por geração: a chave do cache embute contadores global e por usuário, incrementados quando Role/RolePermission/UserRole/PageObject/Página mudam. A invalidação é instantânea em qualquer driver de cache — inclusive revogações em massa. <code class="ptah-c-kbd border rounded px-1 text-xs font-mono">php artisan cache:clear</code> nunca é necessário no fluxo normal.',
    'guide_faq_q6' => 'Posso criar Páginas e Objetos automaticamente via código?',
    'guide_faq_a6' => 'Sim. Use o seeder ou crie registros em <code class="ptah-c-kbd border rounded px-1 text-xs font-mono">Ptah\Models\PtahPage</code> e <code class="ptah-c-kbd border rounded px-1 text-xs font-mono">Ptah\Models\PageObject</code> diretamente (não <code class="ptah-c-kbd border rounded px-1 text-xs font-mono">Ptah\Models\Page</code> — essa classe não existe). Prefira um seeder ou <code class="ptah-c-kbd border rounded px-1 text-xs font-mono">php artisan ptah:permission:sync</code> a uma migration de dados: as migrations do pacote são auto-descobertas e disparam em qualquer <code class="ptah-c-kbd border rounded px-1 text-xs font-mono">php artisan migrate</code> do host, mesmo quando o recurso não é usado.',
